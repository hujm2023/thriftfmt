...
type parser struct {
	...
	DefinitionReservedEndlineComment string
	AllElems                         []ThriftFormatter
}
...
// ParseFileAsAST parses a thrift file and returns an AST.
// If recursive is true, then the include IDLs are parsed recursively as well.
func ParseFileAsAST(path string, includeDirs []string, recursive bool) (*ThriftIDL, error) {
	// if recursive {
	// 	thriftMap := make(map[string]*Thrift)
	// 	dir := filepath.Dir(normalizeFilename(path))
	// 	return parseFileRecursively(path, dir, includeDirs, thriftMap)
	...
	return parseAST(path, string(bs), includeDirs)
}

// ParseFileAsFormatters parses a thrift file and returns an AST.
// If recursive is true, then the include IDLs are parsed recursively as well.
func ParseFileAsFormatters(path string, includeDirs []string, recursive bool) ([]ThriftFormatter, error) {
	// if recursive {
	// 	thriftMap := make(map[string]*Thrift)
	// 	dir := filepath.Dir(normalizeFilename(path))
	// 	return parseFileRecursively(path, dir, includeDirs, thriftMap)
	...
	return parseFormatters(path, string(bs), includeDirs)
}
...
func parseString(path, content string, includeDirs []string) (*Thrift, error) {
	p := &parser{
		...
		AllElems:    make([]ThriftFormatter, 0),
	}
	p.Filename = path
	p.Buffer = content
	_ = p.Init()
...
func parseAST(path, content string, includeDirs []string) (*ThriftIDL, error) {
	p := &parser{
		IncludeDirs: includeDirs,
	}
	p.Filename = path
	...
	return &p.ThriftIDL, nil
}

func parseFormatters(path, content string, includeDirs []string) ([]ThriftFormatter, error) {
	p := &parser{
		IncludeDirs: includeDirs,
	}
	p.Filename = path
...
func (p *parser) parseInclude(node *node32) (err error) {
	...
	pa := &Include{Path: filename}
	p.Includes = append(p.Includes, pa)
	p.AllElems = append(p.AllElems, &IncludeFormatter{*pa})
	return nil
}
...
func (p *parser) parseCppInclude(node *node32) (err error) {
	...
	includeValue := p.pegText(node)
	p.CppIncludes = append(p.CppIncludes, includeValue)
	p.AllElems = append(p.AllElems, &CppIncludeFormatter{cppInclude: includeValue})
	return nil
}
...
func (p *parser) parseNamespace(node *node32) (err error) {
	...
	p.AllElems = append(p.AllElems, &NamespaceFormatter{ns})
	return nil
}
...
func (p *parser) parseDefinition(node *node32) (err error) {
	...
	if node.pegRule == ruleReservedEndLineComments {
		reservedEndlineComments, err := p.parseReservedEndLineComments(node)
		if err != nil {
			return err
		}
		p.DefinitionReservedEndlineComment = reservedEndlineComments
		node = node.next
	}
	// skip Skip
	if node.pegRule == ruleSkip {
...
func (p *parser) parseConst(node *node32) (err error) {
	...
	p.AllElems = append(p.AllElems, &ConstantFormatter{*c})
	return nil
}
...
func (p *parser) parseTypedef(node *node32) (err error) {
	...
	typd.ReservedEndLineComments = p.DefinitionReservedEndlineComment
	p.Typedefs = append(p.Typedefs, &typd)
	p.Annotations = &typd.Annotations
	p.AllElems = append(p.AllElems, &TypedefFormatter{typd})
	return nil
}
...
func (p *parser) parseEnum(node *node32) (err error) {
	...
	for n := node.next.next; n != nil; n = n.next {
		...
		// if n.pegRule == ruleReservedEndLineComments {
		// 	valueCommentEndLine, err = p.parseReservedEndLineComments(n)
		// 	if err != nil {
		// 		return err
		// 	}
		// 	n = n.next
		...
		if n.pegRule == ruleIdentifier {
			...
			if n.next.pegRule == ruleReservedEndLineComments {
				n = n.next
				v.ReservedEndLineComments, err = p.parseReservedEndLineComments(n)
				if err != nil {
					return err
				}
			}
	...
	e.ReservedEndLineComments = p.DefinitionReservedEndlineComment
	p.Enums = append(p.Enums, e)
	p.Annotations = &e.Annotations
	p.AllElems = append(p.AllElems, &EnumFormatter{*e})
	return nil
}
...
func (p *parser) parseStruct(node *node32) (err error) {
	...
	s.ReservedEndLineComments = p.DefinitionReservedComment
	p.Structs = append(p.Structs, s)
	p.Annotations = &s.Annotations
	p.AllElems = append(p.AllElems, &StructLikeFormatter{*s})
	return nil
}
...
func (p *parser) parseField(node *node32) (field *Field, err error) {
	...
	for ; node != nil; node = node.next {
		...
		case ruleReservedEndLineComments:
			...
			f.ReservedEndLineComments = reservedEndLineComments
			// if f.ReservedComments == "" {
			// 	f.ReservedComments = reservedEndLineComments
			// }
		case ruleFieldId:
...
func (p *parser) parseService(node *node32) (err error) {
	...
	for node = node.next; node != nil; node = node.next {
		...
		if node.pegRule == ruleAnnotations { // ANNOTATIONS
			s.Annotations, err = p.parseAnnotations(node)
			if err != nil {
				return err
			}
		}
	...
	s.ReservedComments = p.DefinitionReservedComment
	s.ReservedEndLineComments = p.DefinitionReservedEndlineComment
	p.Services = append(p.Services, &s)
	p.Annotations = &s.Annotations
	p.AllElems = append(p.AllElems, &ServiceFormatter{s})
	return nil
}
...
func (p *parser) parseReservedComments(node *node32) (ReservedComments string, err error) {
	...
	// (Space / TailComment)*
	var comments []string
	var spaceEffective bool
	for ; node != nil; node = node.next {
		if node.pegRule == ruleComment {
			// 识别\n
			comment := string(p.buffer[int(node.up.begin):int(node.up.end)])
		...
		if node.pegRule == ruleSpace {
			s := string(p.buffer[int(node.up.begin):int(node.up.end)])
			if spaceEffective && s == "\n" {
				comments = append(comments, s)
			}
		}
	}
	rComment := strings.Join(comments, commentSplitter)
	return rComment, nil
}
...
func (p *parser) parseReservedEndLineComments(node *node32) (ReservedComments string, err error) {
	...
	var comments []string
	// for ; node != nil; node = node.next {
	// 	if node.pegRule == ruleEndComment {
	// 		s := string(p.buffer[int(node.up.begin):int(node.up.end)])
	...
	// 	}
	// }
	s := string(p.buffer[int(node.begin):int(node.end)])
	comment := strings.TrimRight(s, "\r\n")
	if strings.HasPrefix(comment, "#") {
		comment = strings.TrimPrefix(comment, "#")
		comment = "//" + comment
